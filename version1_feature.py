...
def create_final_input_output(df_final,model_type):
    ...
    output_values=df_final[['smp_d_minus_1','smp_d_plus_4']]
...
def createDF(user_name, passw, host_IP, database_name,dt):
    ...
    df_final['weekday']=1
        ...
        #weather table
    df_metweather2=pd.read_sql('SELECT * FROM yr_weather_half_hour_forecast', cnx)
    df_metweather2=df_metweather2.drop_duplicates('unix_time')
    df_metweather2.rename(columns={'unix_time': 'unix_date'}, inplace=True)
    df_final=df_final.merge(df_metweather2, how='inner', on=['unix_date']) 
    df_final=df_final.drop_duplicates('unix_date')

    
    #labels_forecast=['hour_interval', 'WindForecastEirgrid','hour','weekday','winddirection_deg_y' ,'windspeed_mps_y','precipitation_value_y','temperature_value_y','pressure_value_y','humidity_y']
    labels_forecast=['hour_interval', 'WindForecastEirgrid','hour','weekday','temperature_value_y','winddirection_code','windspeed_mps_y','precipitation_value_y','pressure_value_y']
    labels_historical_short=['smp_d_minus_1','TSODemandForecast','TSORenewableForecast','INSTRUCTION_CODE']
    labels_historical_long=['sum_power','sum_power','smp_d_plus_4','smp_d_minus_1','FUEL_GAS','FUEL_OTHER_FOSSIL', 
...
if __name__ == '__main__':
    ...
    import time
    dt = datetime.datetime(2019, 7, 27 ,20  ,00 ) 
    ...
    for i in range(0,1,1):
        for model_type in model_list:
            ...
            data = pd.DataFrame()
            df_output = pd.DataFrame(data, columns = ['unix_date','Date', 'Hour', 'hour_interval', 'Model_DAM','Actual_DAM']) 
            df_output['Date']=df_final['dates_x'].iloc[len(df_final)-test_window:]
            df_output['Hour']=df_final['hour'].iloc[len(df_final)-test_window:]
            df_output['hour_interval']=df_final['hour_interval'].iloc[len(df_final)-test_window:]
            df_output['unix_date']=df_final['unix_date'].iloc[len(df_final)-test_window:]
            ...
            df_output['Model_DAM']=pred_test_descaled[:,0]
            ...
            delta=abs(df_output['Actual_DAM']-df_output['Model_DAM'])
