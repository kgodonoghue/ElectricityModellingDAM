...
def create_model_output(model_type,input_values_combined, output_values_range):
        if model_type=='regression':
            ...
            clf.add(Dense(1))
            clf.compile(loss='mae', optimizer='adam')
            history = clf.fit(xtrain, ytrain, epochs=30, batch_size=10, validation_data=(xtrain, ytrain), verbose=1, shuffle=True)
            pyplot.plot(history.history['loss'], label='train')
            pyplot.plot(history.history['val_loss'], label='test')
            pyplot.legend()
            pyplot.show()
        ...
        return pred_train,pred_test,ytrain,ytest


path_dukascopy =r'C:/main_folder/sql_modelling/dukascopy'
root_path = r'C:/main_folder/sql_modelling/live_DAM'
user_name = 'fergus'
passw = 'Uniwhite_8080'
...
model_list=['regression','neural net','lstm'] 
model_list=['RF']
final_performance=[]
lstm_history=96
count=0
...
if __name__ == '__main__':
    ...
    fig.savefig('Hourly Distribution.jpg')
    print(['mean',df_results['Delta'].mean()])
    print(['mean',df_results['Delta'].median()])
    print([df_results.corr()])
    df_results_refine=df_results.iloc[len(df_results)-recent_index:]
